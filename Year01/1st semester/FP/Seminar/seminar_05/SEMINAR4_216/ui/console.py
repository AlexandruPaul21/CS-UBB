...
def add_product_ui(store):
    ...
    # remember that if we had different exception types
    # we could have continued with except ...
    # and catch specific exceptions (e.g. ValidationException, DuplicateException
    # in the same block
    # for now we work with ValueError and work around the issues that arise
    try:
        add_product_to_store(store, denumire, unitati, pret)
