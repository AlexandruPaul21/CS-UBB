from domain.product import get_stoc, get_denumire, create_product
...
def test_add_product_to_store():
    ...
    add_product_to_store(test_store, p1)
    assert (len(get_products_list(test_store)) == 1)

    add_product_to_store(test_store, p2)
...
def test_delete_products_from_store():
    # need to also test this newly added fn
    test_store = setup_store(False)
    p1 = create_product('acadele', 0, 8.5)
    p2 = create_product('jeleuri', 26, 5.6)
...
def test_undo():
    ...
    add_product_to_store(test_store2, p1)

    add_product_to_store(test_store2, p2)
