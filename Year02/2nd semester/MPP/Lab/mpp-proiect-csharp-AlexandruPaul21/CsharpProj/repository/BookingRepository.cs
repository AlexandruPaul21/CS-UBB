...
public class BookingRepository : IBookingRepository
{
    ...
    {
        logger.Info("Booking - getNumberOfBookingsForFlight - Enter");
        var sql = "SELECT COUNT(*) FROM bookings WHERE id_flight = @idFlight";
        
        using (var connection = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["Test"].ConnectionString))
        {
            using (var cmd = new NpgsqlCommand(sql, connection))
            {
                cmd.Parameters.AddWithValue("@idFlight", flight.Id);
                var reader = cmd.ExecuteReader();
                if (reader.Read())
                {
                    var count = reader.GetInt32(0);
                    logger.Info("Booking - getNumberOfBookingsForFlight - Exit");
                    return count;
                }
            }
        }
    ...
    public long getLowestAvailableId()
    {
        ...
        var sql = "SELECT id FROM bookings ORDER BY id";
        long id = 0;
        using (var connection = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["Test"].ConnectionString))
        {
            using (var cmd = new NpgsqlCommand(sql, connection))
